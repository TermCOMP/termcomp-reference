...
We\comment{FF This part is copied from the previous chapter.
  %
  We should avoid duplication.} write $\mathcal{T}(\F,\V)$ for the set of all terms over a signature $\F = \biguplus_{k \in \N} \F_k$ of function symbols and a set $\V$ of variables.
A function symbol $f \in \F_k$ has arity $k$.
As usual, substitutions, positions, subterms, and contexts are defined exactly as for ordinary TRSs, see \Cref{chap:TRS}.
...
\begin{formalproblem}
  ...
  {\texttt{WORST\_CASE(}$f$\texttt{,}$g$\texttt{)}, where}\comment{FF In the other chapters, we use $<$ instead of $\sqsubset$.
    %
    I think we should use the same notation here for consistency.}
  \begin{align*}
    &f \in \{\texttt{?} \sqsubset \texttt{Non-SAST} \sqsubset \texttt{Non-AST}\}\\
    &g \in \{\texttt{?} \sqsubset \texttt{AST} \sqsubset \texttt{SAST}\}
  \end{align*}
...
and \texttt{WORST\_CASE(?,AST)}, \texttt{WORST\_CASE(Non-SAST,?)}, and \texttt{WORST\_CASE(?,?)} 
are the other possible outputs.
...
and \texttt{WORST\_CASE(?,?)} is the other possible output.\comment{FF What about \texttt{WORST\_CASE(?,AST)}?}
...
and \texttt{WORST\_CASE(?,?)} is the other possible output.\comment{FF What about \texttt{WORST\_CASE(Non-SAST,?)}?}
...
\begin{formalproblem}
  ...
  \begin{align*}
    &f \in \{\texttt{?} \sqsubseteq \texttt{Non-SAST} \sqsubseteq \texttt{Non-AST}\}\\
    &g \in \{\texttt{?} \sqsubseteq \texttt{AST} \sqsubseteq \texttt{SAST}\}
  \end{align*}
  and where $f$ is the lower bound and $g$ is the upper bound.
\end{formalproblem}
