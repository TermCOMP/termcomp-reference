...
\]
%
An ITS terminates if all evaluations are finite.
%
The \emph{runtime complexity} is defined as follows.
%
\begin{definition}[Runtime Complexity]
The \emph{runtime complexity} is the function $\rc\colon\Sigma\rightarrow\N \cup \{\omega\}$ with
\[
  \rc(\sigma) = \sup \{k \in \N \mid \exists s \in \CC.\ \mathit{entry}(x_1, \ldots, x_n)\sigma \rightarrow^k s \}.
\]
\end{definition}
%
\noindent
...
%
Formally, let $|\sigma| = \max \{ |\sigma(x_j)| \mid 1 \leq i \leq n \}$ denote the largest absolute initial value among all program variables $x_1,\dots,x_n$ of $\mathit{entry}(x_1,\ldots,x_n)$. In the output of the tool, $|\sigma|$ is represented by ``\texttt{n}''.
%
For the lower bound \texttt{LB}, the possible outputs and their semantics are:
\begin{itemize}
    \item \texttt{?}: No lower bound could be established.
    \item \text{\texttt{Omega(n\textasciicircum}$k$\texttt{)}}: A polynomial lower bound of degree $k \in \N$, i.e., $lb(\sigma) \in \Omega(|\sigma|^k)$.
      %
      The output \texttt{Omega(n)} is equivalent to \texttt{Omega(n\textasciicircum 1)}.
      %
      The output \texttt{Omega(1)} is equivalent to \texttt{?}.
...
\begin{itemize}
    ...
    \item \text{\texttt{O(n\textasciicircum}$k$\texttt{)}}: A polynomial upper bound of degree $k \in \N$, i.e., $rb(\sigma) \in \mathcal{O}(|\sigma|^k)$.
      %
      The output \texttt{Omega(n)} is equivalent to \texttt{Omega(n\textasciicircum 1)}.
      %
      The output \texttt{Omega(1)} is equivalent to \texttt{Omega(n\textasciicircum 0)}.
