...
%
Formally, let $||\sigma||_{\infty} = \max \{ |\sigma(x_j)| \mid 1 \leq i \leq n \}$ denote the maximums norm of the state $\sigma\in\Sigma$, i.e., the largest absolute initial value among all program variables $x_1,\dots,x_n$ of $\mathit{entry}(x_1,\ldots,x_n)$.
%
Note that Boolean values $\bot$ and $\top$ are considered as integer values $0$ and $1$, respectively.
%
In the output of the tool, $||\sigma||_{\infty}$ is represented by ``\texttt{n}''.
%
For the lower bound \texttt{LB}, the possible outputs and their semantics are:
\begin{itemize}
    \item \texttt{?}: No lower bound could be established.
    \item \text{\texttt{Omega(n\textasciicircum}$k$\texttt{)}}: A polynomial lower bound of degree $k \in \N$, i.e., $rc(\sigma) \in \Omega(||\sigma||_{\infty}^k)$.
      %
      \begin{itemize}
...
\begin{itemize}
    ...
    \item \text{\texttt{O(n\textasciicircum}$k$\texttt{)}}: A polynomial upper bound of degree $k \in \N$, i.e., $rc(\sigma) \in \mathcal{O}(||\sigma||_{\infty}^k)$.
      \begin{itemize}
